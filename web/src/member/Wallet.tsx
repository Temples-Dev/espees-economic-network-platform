  ...
  return (
    <div className="flex items-start justify-between gap-3 py-1.5">
      <div>
        ...
        {!available && <p className="text-xs text-zinc-500">{hint}</p>}
      </div>
      <span
        className={`rounded-full px-2 py-0.5 text-xs ${
          available ? "bg-emerald-950 text-emerald-300" : "bg-zinc-800 text-zinc-400"
        }`}
      >
      ...
      <Card>
        ...
        {wallet ? (
          <div className="mt-2 space-y-1 text-sm">
            ...
            {wallet.status_detail && <p className="text-xs text-zinc-500">{wallet.status_detail}</p>}
